...
-- Read Auth's current email every time access is checked. A profile's owner
-- role must not survive an out-of-band email change to another approved inbox.
create or replace function public.code_city_profile_identity_allowed(p_user_id uuid, p_role text)
returns boolean language sql stable security definer set search_path = '' as $$
  select public.code_city_admin_email_allowed(p_user_id)
    and coalesce(p_role in ('owner', 'admin', 'agent', 'viewer'), false)
    and (p_role <> 'owner' or exists (
      select 1 from auth.users u
      where u.id = p_user_id and lower(u.email) = 'dev@codecity.ai'
    ));
$$;
revoke all on function public.code_city_profile_identity_allowed(uuid, text) from public, anon, authenticated;
grant execute on function public.code_city_profile_identity_allowed(uuid, text) to authenticated, service_role;
...
do $$ begin
  if exists (
    ...
    where p.is_active and p.role = 'owner'
      and not public.code_city_profile_identity_allowed(p.user_id, p.role)
  ) then raise exception 'Active Code City owner email is not approved'; end if;
end $$;

update public.admin_profiles p
set is_active = false
where p.is_active and not public.code_city_profile_identity_allowed(p.user_id, p.role);
...
begin
  if new.role = 'owner' and not public.code_city_profile_identity_allowed(new.user_id, new.role) then
    raise exception 'Only the Code City owner email may hold the owner role' using errcode = '23514';
  end if;
  if new.is_active and not public.code_city_profile_identity_allowed(new.user_id, new.role) then
...
alter policy admin_profiles_identity_self_select on public.admin_profiles
  using (user_id = (select auth.uid()) and is_active
    and public.code_city_profile_identity_allowed(user_id, role));
...
begin
  ...
  return exists (
    ...
    where g.user_id = v_user and g.session_id = v_session::uuid and g.project = p_project
      ...
      and public.code_city_profile_identity_allowed(a.user_id, a.role)
