...
| ORC selection | Checks the `orc` project, then offers `/orc/` after unlock. |
...
The task's local CUA verification covered routing, focus return, and isolated project-code UI behavior using `scripts/verify-project-access.mjs`. The implementation handoff reports 71 JavaScript tests, 19 Python tests, and SQL checks passing; this documentation pass did not rerun those suites. Fixture interactions send no real email and do not open a live trading session.
